# coding-executor plan — agent/backend-engineer/issue-5-coding-execute

- session_id: `166c416a1ed0`
...
repo: https://github.com/yule-studio/naver-search-clone.git  이전 planning / record-only 세션과 draft PR은 폐기하고 새 canonical session으로 다시 시작해줘.  제품 목표: - 네이버 검색 경험을 강하게 참고한 풀스택 MVP 구현 - 다만 로고, 상표, 아이콘, 문구, 이미지의 1:1 복제는 피하고 자체 구현으로 대체  구현 범위: - 회원가입 - 로그인 - 로그아웃 - 인증 사용자만 접근 가능 - 검색 홈 화면 - 검색 결과 탭: 통합 / 블로그 / 메일 - 블로그: 목록 / 상세 / 작성 - 메일: inbox / sent / detail / compose - 검색 대상은 앱 내부 DB 데이터 - /health 유지 - docker compose로 web/api/db 함께 실행 - seed data 포함 - 실제 실행 가능한 상태까지 구현  작업 규칙: - 반드시 GitHub issue를 먼저 만들고 issue anchor 기준으로 시작 - Git Flow branch 전략 사용 - autonomous_merge 모드로 gateway가 종료 시점까지 end-to-end control - planning-only PR이 아니라 실제 구현 경로를 우선 - 외부 정보/secret/access 필요 시에만 승인 카드 사용  우선순위 slice: 1. auth backend 2. auth frontend 3. search home UI 4. integrated search API + result tabs 5. blog core 6. mail core 7. docker/dev runtime stabilization 8. basic tests and polish
